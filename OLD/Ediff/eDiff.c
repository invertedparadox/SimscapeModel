...
float pow_diy(float base, uint8_t power)
{
    float output = base;
...
void eDiff(float AP, float left_wheel_speed, float right_wheel_speed,
            ...
            float yaw, float* filter_yaw, int16_t* torque_output_left, int16_t* torque_output_right)
{
    ...
    vehicle_speed = (left_wheel_speed + right_wheel_speed) / 2.0;
    ...
    *filter_yaw = ((yaw) * RATE_LIMITER ) + ((1 - RATE_LIMITER ) * *filter_yaw);
    ...
    // pre-PID
    desired_wheel_speed_diff_steer = vehicle_speed * STEERING_W * tan(avg_steer_angle) * STEERING_K_STEER / (STEERING_L);
    ...
    if(*torque_output_right < (- 0 * LV_MAX_ADC_RAW))
    {
        *torque_output_right = (- 0 * LV_MAX_ADC_RAW);
    }

    // limit regen torque (temporary)
    if(*torque_output_left < (- 0 * LV_MAX_ADC_RAW))
    {
        *torque_output_left = (- 0 * LV_MAX_ADC_RAW);
    }
