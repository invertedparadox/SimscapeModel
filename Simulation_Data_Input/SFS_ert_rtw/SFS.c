...
void SFS_step(RT_MODEL *const rtM, ExtU *rtU, ExtY *rtY)
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    rtY->acc_VNED[i] = (IMUNED2VNED_tmp * rtU->acc[1] + IMUNED2VNED[i] *
    ...
    rtY->angvel_VNED[i] = IMUNED2VNED_tmp;
    rtb_angvel_VNED[i] = IMUNED2VNED_tmp;
  }
  ...
  for (i = 0; i < 3; i++) {
     ...
     *  Sum: '<S1>/Sum'
     */
    rtDW->UnitDelay_DSTATE[i] = rtb_angvel_VNED[i];
  }
